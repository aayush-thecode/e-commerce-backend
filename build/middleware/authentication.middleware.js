...
const Authenticate = (roles) => {
    return (req, res, next) => __awaiter(void 0, void 0, void 0, function* () {
        try {
            ...
            if (!access_token) {
                throw new errorhandler_middleware_1.CustomError('Unauthorized, token is missing! ,access denied!', 401);
            }
            const decoded = (0, jwt_utils_1.verifyToken)(access_token);
            if (decoded.exp && decoded.exp * 1000 < Date.now()) {
                throw new errorhandler_middleware_1.CustomError('Unauthorized, access denied', 401);
            }
            ...
            if (roles && !roles.includes(user.role)) {
                throw new errorhandler_middleware_1.CustomError(`Forbidden, ${user.role} can not acess this resource`, 401);
            }
            //ts-expect-error
            req.user = {
                _id: decoded._id,
